    ...
    def save_result_redis(self, result):
        ...
        if result.get("ch"):
            ...
            for data in data_list:
                ...
                item["Amount"] = float(data.get("amount") if data.get("amount") else 0)
                # Volume 成交额(USDT)
                item["Volume"] = usdt_price * item["Amount"]
                # print(item)

                redis_key_name = "huobipro:spot:trade:{}_{}_trade_detail".format(self.pair1, self.pair2)
                while True:
                    try:
    ...
    if pair_url_spot:
        ...
        for trade_info in trade_info_spot:
            ...
            for index, symbol in enumerate(symbol_list):
                ...
                spider = HuobiProTradeSpider(logger, symbol, exchange, req, trade_type, pair1, pair2)
