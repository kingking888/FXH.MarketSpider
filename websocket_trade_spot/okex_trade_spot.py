    ...
    def save_result_redis(self, result):
        ...
        if result.get("table"):
            ...
            for data in data_list:
                ...
                item["Amount"] = float(data.get("qty") if data.get("qty") else data.get("size"))
                # volume: 成交量(张)，买卖双边成交量之和
                item["Volume"] = float(item["UsdtPrice"] * item["Amount"])

                redis_key_name = "okex:spot:trade:{}_{}_trade_detail".format(self.pair1, self.pair2)
        ...
        if pair_url_spot:
            ...
            for trade_info in trade_info_spot:
                ...
                for index, symbol in enumerate(symbol_list):
                    ...
                    req = "{" + trade[1: -1].format(symbol=symbol[0]) + "}"
                    pair1 = symbol[1].upper()
                    pair2 = symbol[2].upper()
                    spider = OkexTradeSpider(logger, symbol[0], exchange, req, trade_type, pair1, pair2)
