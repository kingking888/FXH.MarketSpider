...
class StockMarket(object):
    ...
    def get_n225(self):
        ...
        item = {
            ...
            'increase_rate': data1[2].replace("%", "").replace("-", "-%"),
            'increase_amount': float(data1[1].replace(",", "")),
            'amplitude': "{}{:.2f}".format("%", ((_high - _low) / _close * 100)),
            'open': _open,
            'close': _close,
            'high': _high,
            'low': _low,
    ...
    def get_xau(self):
        ...
        item = {
            'name': 'XAU',
            'price': _price,
            'increase_rate': float("{:.2f}".format(_price - _close)),
            'increase_amount': "{}{:.2f}".format("%", (_price - _close) / _close * 100).replace("%-", "-%"),
            'amplitude': "{}{:.2f}".format("%", (_high - _low) / _close * 100).replace("%-", "-%"),
            'open': _open,
            'close': _close,
            'high': _high,
            'low': _low,
